...
use honeycomb_core::{DartIdentifier, FloatType, SewPolicy, TwoMap, UnsewPolicy};
...
fn split<const N_MARKS: usize>(mut map: TwoMap<N_MARKS, FloatType>) {
...
fn split_some<const N_MARKS: usize>(mut map: TwoMap<N_MARKS, FloatType>, split: &[bool]) {
    (0..N_SQUARE.pow(2))
...
fn split_diff<const N_MARKS: usize>(mut map: TwoMap<N_MARKS, FloatType>, split: &[bool]) {
    (0..N_SQUARE.pow(2)).for_each(|square| {
