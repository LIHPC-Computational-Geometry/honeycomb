...
use honeycomb::prelude::{VertexId, NULL_DART_ID};
use honeycomb_benches::FloatType;
use honeycomb_core::prelude::{CMap2, CMapBuilder, DartId, Vertex2};
use iai_callgrind::{
...
fn get_sparse_map(n_square: usize) -> CMap2<FloatType> {
    ...
    map.set_betas(DartId(5), [NULL_DART_ID; 3]); // free dart 5
    map.remove_free_dart(DartId(5));
...
fn add_single_dart(map: &mut CMap2<FloatType>) -> DartId {
...
#[bench::medium(&mut get_map(64))]
#[bench::large(&mut get_map(256))]
fn add_ten_darts(map: &mut CMap2<FloatType>) -> DartId {
    black_box(map.add_free_darts(10))
}

#[library_benchmark]
#[benches::with_setup(args = [16, 32, 64, 128, 256, 512], setup = get_empty_map)]
fn add_many_darts((mut map, n_darts): (CMap2<FloatType>, usize)) -> DartId {
...
fn insert_dart(map: &mut CMap2<FloatType>) -> DartId {
...
#[bench::medium(&mut get_map(64))]
#[bench::large(&mut get_map(256))]
fn insert_dart_full(map: &mut CMap2<FloatType>) -> DartId {
    black_box(map.insert_free_dart())
}
...
fn read_vertex(map: &mut CMap2<FloatType>) -> Option<Vertex2<FloatType>> {
    black_box(map.vertex(VertexId(1)))
}
...
fn replace_vertex(map: &mut CMap2<FloatType>) -> Option<Vertex2<FloatType>> {
    black_box(map.replace_vertex(VertexId(1), (0.0, 0.0)))
}
...
fn remove_vertex(map: &mut CMap2<FloatType>) -> Option<Vertex2<FloatType>> {
    black_box(map.remove_vertex(VertexId(1)))
}
