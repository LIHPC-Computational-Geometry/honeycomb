...
use honeycomb_core::{
    cmap::DartIdType,
    prelude::{CMap2, CoordsFloat, DartId, Vertex2, NULL_DART_ID},
};
use std::{
    cmp::{max, min},
            ...
            _ => {
                ...
                match diff {
                    (i, 0) => {
                        ...
                        let tmp =
                            ...
                            (min(i_base, i_base + 1 + i)..max(i_base + i, i_base + 1)).zip(i_ids).map(|(x, id)| {
                                ...
                                // intersected dart
                                let dart_id = DartId(if i.is_positive() {
                                    d_base + 1
                                } else {
                                    d_base + 3
                                });
                    ...
                    (0, j) => {
                        ...
                        let tmp =
                            ...
                            (min(j_base, j_base + 1 + j)..max(j_base + j, j_base + 1)).zip(i_ids).map(|(y, id)| {
                                ...
                                // intersected dart
                                let dart_id = DartId(if j.is_positive() { d_base + 2 } else { d_base });
                                // vertex associated to the intersected dart
                                let v_dart = cmap.vertex(cmap.vertex_id(dart_id))
                                    .expect("E: found a topological vertex with no associated coordinates");
                                // compute intersection
                    ...
                    (i, j) => {
                        ...
                        let mut intersec_data: Vec<(T, T, DartId)> = subgrid_cells
                            .map(|(x, y)| {
                                // cell base dart
                                let d_base = (1 + 4 * x + nx as isize * 4 * y) as DartIdType;
                                // (potentially) intersected darts
                                let vdart_id = DartId(if i.is_positive() {
                                    d_base + 1
                                } else {
                                    d_base + 3
                                });
                                let hdart_id = DartId(if j.is_positive() { d_base + 2 } else { d_base });
                                // associated vertices
