...
use applications::{
    FileFormat, bind_rayon_threads, finalize_3d, get_num_threads, init_3d_map_from_file,
...
fn run_bench<T: CoordsFloat>(
    ...
    let (map, input_hash, init_time) = init_3d_map_from_file::<T>(input.clone());

    println!("| 3D shift benchmark");
    println!(
