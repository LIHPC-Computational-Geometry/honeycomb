// ------ IMPORTS

use crate::{
    attributes::AttrSparseVec,
    cmap::{DartId, EdgeId, FaceId, VertexId},
    prelude::{AttributeBind, AttributeUpdate, CMap2, CMapBuilder, Orbit2, OrbitPolicy, Vertex2},
};

// ------ CONTENT

// --- GENERAL

#[test]
fn example_test() {
    // build a triangle
    let mut map: CMap2<f64> = CMapBuilder::default().n_darts(3).build().unwrap();
    map.one_link(DartId(1), DartId(2));
    map.one_link(DartId(2), DartId(3));
    map.one_link(DartId(3), DartId(1));
    map.insert_vertex(VertexId(1), (0.0, 0.0));
    map.insert_vertex(VertexId(2), (1.0, 0.0));
    map.insert_vertex(VertexId(3), (0.0, 1.0));

    // checks
    let faces = map.fetch_faces();
    assert_eq!(faces.identifiers.len(), 1);
    assert_eq!(faces.identifiers[0], FaceId(1));
    let mut face = Orbit2::new(&map, OrbitPolicy::Face, DartId(1));
    assert_eq!(face.next(), Some(DartId(1)));
    assert_eq!(face.next(), Some(DartId(2)));
    assert_eq!(face.next(), Some(DartId(3)));
    assert_eq!(face.next(), None);

    // build a second triangle
    map.add_free_darts(3);
    map.one_link(DartId(4), DartId(5));
    map.one_link(DartId(5), DartId(6));
    map.one_link(DartId(6), DartId(4));
    map.insert_vertex(VertexId(4), (0.0, 2.0));
    map.insert_vertex(VertexId(5), (2.0, 0.0));
    map.insert_vertex(VertexId(6), (1.0, 1.0));

    // checks
    let faces = map.fetch_faces();
    assert_eq!(&faces.identifiers, &[FaceId(1), FaceId(4)]);
    let mut face = Orbit2::new(&map, OrbitPolicy::Face, DartId(4));
    assert_eq!(face.next(), Some(DartId(4)));
    assert_eq!(face.next(), Some(DartId(5)));
    assert_eq!(face.next(), Some(DartId(6)));
    assert_eq!(face.next(), None);

    // sew both triangles
    map.two_sew(DartId(2), DartId(4));

    // checks
    assert_eq!(map.beta::<2>(DartId(2)), DartId(4));
    assert_eq!(map.vertex_id(DartId(2)), VertexId(2));
    assert_eq!(map.vertex_id(DartId(5)), VertexId(2));
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((1.5, 0.0)));
    assert_eq!(map.vertex_id(DartId(3)), VertexId(3));
    assert_eq!(map.vertex_id(DartId(4)), VertexId(3));
    assert_eq!(map.vertex(VertexId(3)).unwrap(), Vertex2::from((0.0, 1.5)));
    let edges = map.fetch_edges();
    assert_eq!(
        &edges.identifiers,
        &[EdgeId(1), EdgeId(2), EdgeId(3), EdgeId(5), EdgeId(6)]
    );

    // adjust bottom-right & top-left vertex position
    assert_eq!(
        map.replace_vertex(VertexId(2), Vertex2::from((1.0, 0.0))),
        Some(Vertex2::from((1.5, 0.0)))
    );
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((1.0, 0.0)));
    assert_eq!(
        map.replace_vertex(VertexId(3), Vertex2::from((0.0, 1.0))),
        Some(Vertex2::from((0.0, 1.5)))
    );
    assert_eq!(map.vertex(VertexId(3)).unwrap(), Vertex2::from((0.0, 1.0)));

    // separate the diagonal from the rest
    map.one_unsew(DartId(1));
    map.one_unsew(DartId(2));
    map.one_unsew(DartId(6));
    map.one_unsew(DartId(4));
    // break up & remove the diagonal
    map.two_unsew(DartId(2)); // this makes dart 2 and 4 free
    map.remove_free_dart(DartId(2));
    map.remove_free_dart(DartId(4));
    // sew the square back up
    map.one_sew(DartId(1), DartId(5));
    map.one_sew(DartId(6), DartId(3));

    // i-cells
    let faces = map.fetch_faces();
    assert_eq!(&faces.identifiers, &[FaceId(1)]);
    let edges = map.fetch_edges();
    assert_eq!(
        &edges.identifiers,
        &[EdgeId(1), EdgeId(3), EdgeId(5), EdgeId(6)]
    );
    let vertices = map.fetch_vertices();
    assert_eq!(
        &vertices.identifiers,
        &[VertexId(1), VertexId(3), VertexId(5), VertexId(6)]
    );
    assert_eq!(map.vertex(VertexId(1)).unwrap(), Vertex2::from((0.0, 0.0)));
    assert_eq!(map.vertex(VertexId(5)).unwrap(), Vertex2::from((1.0, 0.0)));
    assert_eq!(map.vertex(VertexId(6)).unwrap(), Vertex2::from((1.0, 1.0)));
    assert_eq!(map.vertex(VertexId(3)).unwrap(), Vertex2::from((0.0, 1.0)));
    // darts
    assert_eq!(map.n_unused_darts(), 2); // there are unused darts since we removed the diagonal
    assert_eq!(map.beta_runtime(1, DartId(1)), DartId(5));
    assert_eq!(map.beta_runtime(1, DartId(5)), DartId(6));
    assert_eq!(map.beta_runtime(1, DartId(6)), DartId(3));
    assert_eq!(map.beta_runtime(1, DartId(3)), DartId(1));
}

#[test]
#[should_panic(expected = "called `Option::unwrap()` on a `None` value")]
fn remove_vertex_twice() {
    // in its default state, all darts/vertices of a map are considered to be used
    let mut map: CMap2<f64> = CMap2::new(4);
    // set vertex 1 as unused
    map.remove_vertex(VertexId(1)).unwrap();
    // set vertex 1 as unused, again
    map.remove_vertex(VertexId(1)).unwrap(); // this should panic
}

#[test]
#[should_panic(expected = "assertion failed")]
fn remove_dart_twice() {
    // in its default state, all darts/vertices of a map are considered to be used
    // darts are also free
    let mut map: CMap2<f64> = CMap2::new(4);
    // set dart 1 as unused
    map.remove_free_dart(DartId(1));
    // set dart 1 as unused, again
    map.remove_free_dart(DartId(1)); // this should panic
}

// --- (UN)SEW

#[test]
fn two_sew_complete() {
    let mut map: CMap2<f64> = CMap2::new(4);
    map.one_link(DartId(1), DartId(2));
    map.one_link(DartId(3), DartId(4));
    map.insert_vertex(VertexId(1), (0.0, 0.0));
    map.insert_vertex(VertexId(2), (0.0, 1.0));
    map.insert_vertex(VertexId(3), (1.0, 1.0));
    map.insert_vertex(VertexId(4), (1.0, 0.0));
    map.two_sew(DartId(1), DartId(3));
    assert_eq!(map.vertex(VertexId(1)).unwrap(), Vertex2::from((0.5, 0.0)));
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((0.5, 1.0)));
}

#[test]
fn two_sew_incomplete() {
    let mut map: CMap2<f64> = CMap2::new(3);
    map.one_link(DartId(1), DartId(2));
    map.insert_vertex(VertexId(1), (0.0, 0.0));
    map.insert_vertex(VertexId(2), (0.0, 1.0));
    map.insert_vertex(VertexId(3), (1.0, 1.0));
    map.two_sew(DartId(1), DartId(3));
    // missing beta1 image for dart 3
    assert_eq!(map.vertex(VertexId(1)).unwrap(), Vertex2::from((0.0, 0.0)));
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((0.5, 1.0)));
    map.two_unsew(DartId(1));
    assert_eq!(map.add_free_dart(), DartId(4));
    map.one_link(DartId(3), DartId(4));
    map.two_sew(DartId(1), DartId(3));
    // missing vertex for dart 4
    assert_eq!(map.vertex(VertexId(1)).unwrap(), Vertex2::from((0.0, 0.0)));
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((0.5, 1.0)));
}

#[test]
fn two_sew_no_b1() {
    let mut map: CMap2<f64> = CMap2::new(2);
    map.insert_vertex(VertexId(1), (0.0, 0.0));
    map.insert_vertex(VertexId(2), (1.0, 1.0));
    map.two_sew(DartId(1), DartId(2));
    assert_eq!(map.vertex(VertexId(1)).unwrap(), Vertex2::from((0.0, 0.0)));
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((1.0, 1.0)));
}

#[test]
// #[should_panic] // FIXME: find a way to test what's printed?
fn two_sew_no_attributes() {
    let mut map: CMap2<f64> = CMap2::new(2);
    map.two_sew(DartId(1), DartId(2)); // should panic
}

#[test]
// #[should_panic] // FIXME: find a way to test what's printed?
fn two_sew_no_attributes_bis() {
    let mut map: CMap2<f64> = CMap2::new(4);
    map.one_link(DartId(1), DartId(2));
    map.one_link(DartId(3), DartId(4));
    map.two_sew(DartId(1), DartId(3)); // panic
}

#[test]
#[should_panic(expected = "Dart 1 and 3 do not have consistent orientation for 2-sewing")]
fn two_sew_bad_orientation() {
    let mut map: CMap2<f64> = CMap2::new(4);
    map.one_link(DartId(1), DartId(2));
    map.one_link(DartId(3), DartId(4));
    map.insert_vertex(VertexId(1), (0.0, 0.0));
    map.insert_vertex(VertexId(2), (0.0, 1.0)); // 1->2 goes up
    map.insert_vertex(VertexId(3), (1.0, 0.0));
    map.insert_vertex(VertexId(4), (1.0, 1.0)); // 3->4 also goes up
    map.two_sew(DartId(1), DartId(3)); // panic
}

#[test]
fn one_sew_complete() {
    let mut map: CMap2<f64> = CMap2::new(3);
    map.two_link(DartId(1), DartId(2));
    map.insert_vertex(VertexId(1), (0.0, 0.0));
    map.insert_vertex(VertexId(2), (0.0, 1.0));
    map.insert_vertex(VertexId(3), (0.0, 2.0));
    map.one_sew(DartId(1), DartId(3));
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((0.0, 1.5)));
}

#[test]
fn one_sew_incomplete_attributes() {
    let mut map: CMap2<f64> = CMap2::new(3);
    map.two_link(DartId(1), DartId(2));
    map.insert_vertex(VertexId(1), (0.0, 0.0));
    map.insert_vertex(VertexId(2), (0.0, 1.0));
    map.one_sew(DartId(1), DartId(3));
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((0.0, 1.0)));
}

#[test]
fn one_sew_incomplete_beta() {
    let mut map: CMap2<f64> = CMap2::new(3);
    map.insert_vertex(VertexId(1), (0.0, 0.0));
    map.insert_vertex(VertexId(2), (0.0, 1.0));
    map.one_sew(DartId(1), DartId(2));
    assert_eq!(map.vertex(VertexId(2)).unwrap(), Vertex2::from((0.0, 1.0)));
}
#[test]
// #[should_panic] // FIXME: find a way to test what's printed?
fn one_sew_no_attributes() {
    let mut map: CMap2<f64> = CMap2::new(2);
    map.one_sew(DartId(1), DartId(2)); // should panic
}

#[test]
// #[should_panic] // FIXME: find a way to test what's printed?
fn one_sew_no_attributes_bis() {
    let mut map: CMap2<f64> = CMap2::new(3);
    map.two_link(DartId(1), DartId(2));
    map.one_sew(DartId(1), DartId(3)); // panic
}

// --- IO

#[cfg(feature = "io")]
#[test]
fn io_write() {
    // build a map looking like this:
    //      15
    //     / \
    //    /   \
    //   /     \
    //  16      14
    //  |       |
    //  4---3---7
    //  |   |  /|
    //  |   | / |
    //  |   |/  |
    //  1---2---6
    let mut cmap: CMap2<f32> = CMap2::new(16);
    // bottom left square
    cmap.one_link(DartId(1), DartId(2));
    cmap.one_link(DartId(2), DartId(3));
    cmap.one_link(DartId(3), DartId(4));
    cmap.one_link(DartId(4), DartId(1));
    // bottom right triangles
    cmap.one_link(DartId(5), DartId(6));
    cmap.one_link(DartId(6), DartId(7));
    cmap.one_link(DartId(7), DartId(5));
    cmap.two_link(DartId(7), DartId(8));
    cmap.one_link(DartId(8), DartId(9));
    cmap.one_link(DartId(9), DartId(10));
    cmap.one_link(DartId(10), DartId(8));
    // top polygon
    cmap.one_link(DartId(11), DartId(12));
    cmap.one_link(DartId(12), DartId(13));
    cmap.one_link(DartId(13), DartId(14));
    cmap.one_link(DartId(14), DartId(15));
    cmap.one_link(DartId(15), DartId(16));
    cmap.one_link(DartId(16), DartId(11));
    // assemble
    cmap.two_link(DartId(2), DartId(10));
    cmap.two_link(DartId(3), DartId(11));
    cmap.two_link(DartId(9), DartId(12));

    // insert vertices
    cmap.insert_vertex(VertexId(1), (0.0, 0.0));
    cmap.insert_vertex(VertexId(2), (1.0, 0.0));
    cmap.insert_vertex(VertexId(6), (2.0, 0.0));
    cmap.insert_vertex(VertexId(4), (0.0, 1.0));
    cmap.insert_vertex(VertexId(3), (1.0, 1.0));
    cmap.insert_vertex(VertexId(7), (2.0, 1.0));
    cmap.insert_vertex(VertexId(16), (0.0, 2.0));
    cmap.insert_vertex(VertexId(15), (1.0, 3.0));
    cmap.insert_vertex(VertexId(14), (2.0, 2.0));

    // generate VTK data
    let mut res = String::new();
    cmap.to_vtk_ascii(&mut res);
    println!("{res}");

    // check result
    assert!(res.contains("POINTS 9 float"));
    assert!(res.contains("CELLS 12 44"));
    assert!(res.contains("CELL_TYPES 12"));
    // faces
    assert!(res.contains("4 0 1 2 3"));
    assert!(res.contains("3 1 4 5"));
    assert!(res.contains("4 0 1 2 3"));
    assert!(res.contains("4 0 1 2 3"));
    // edges
    assert!(res.contains("2 0 1"));
    assert!(res.contains("2 3 0"));
    assert!(res.contains("2 1 4"));
    assert!(res.contains("2 4 5"));
    assert!(res.contains("2 5 6"));
    assert!(res.contains("2 6 7"));
    assert!(res.contains("2 7 8"));
    assert!(res.contains("2 8 3"));
}

// --- PARALLEL

#[derive(Debug, Clone, Copy, Default)]
struct Weight(pub u32);

impl AttributeUpdate for Weight {
    fn merge(attr1: Self, attr2: Self) -> Self {
        Self(attr1.0 + attr2.0)
    }

    fn split(attr: Self) -> (Self, Self) {
        // adding the % to keep things conservative
        (Weight(attr.0 / 2 + attr.0 % 2), Weight(attr.0 / 2))
    }
}

impl AttributeBind for Weight {
    type StorageType = AttrSparseVec<Self>;
    type IdentifierType = VertexId;
    const BIND_POLICY: OrbitPolicy = OrbitPolicy::Vertex;
}

#[test]
fn sew_ordering() {
    loom::model(|| {
        // setup the map
        let map: CMap2<f64> = CMapBuilder::default().n_darts(5).build().unwrap();
        map.two_link(DartId(1), DartId(2));
        map.one_link(DartId(4), DartId(5));
        map.insert_vertex(VertexId(2), Vertex2(1.0, 1.0));
        map.insert_vertex(VertexId(3), Vertex2(1.0, 2.0));
        map.insert_vertex(VertexId(5), Vertex2(2.0, 2.0));
        let arc = loom::sync::Arc::new(map);
        let (m1, m2) = (arc.clone(), arc.clone());

        // we're going to do to sew ops:
        // - 1-sew 1 to 3 (t1)
        // - 2-sew 3 to 4 (t2)
        // this will result in a single vertex being define, of ID 2
        // depending on the order of execution of the sews, the value may change
        // 1-sew before 2-sew: (1.5, 1.75)
        // 2-sew before 1-sew: (1.25, 1.5)

        let t1 = loom::thread::spawn(move || {
            m1.atomically_one_sew(DartId(1), DartId(3));
        });

        let t2 = loom::thread::spawn(move || {
            m2.atomically_two_sew(DartId(3), DartId(4));
        });

        t1.join().unwrap();
        t2.join().unwrap();

        // all path should result in the same topological result here
        assert!(arc.vertex(VertexId(2)).is_some());
        assert!(arc.vertex(VertexId(3)).is_none());
        assert!(arc.vertex(VertexId(5)).is_none());
        assert_eq!(
            Orbit2::new(arc.as_ref(), OrbitPolicy::Vertex, DartId(2)).count(),
            3
        );

        // the v2 can have two values though
        let path1 = arc.vertex(VertexId(2)) == Some(Vertex2(1.5, 1.75));
        let path2 = arc.vertex(VertexId(2)) == Some(Vertex2(1.25, 1.5));
        assert!(path1 || path2);
    });
}

#[test]
fn unsew_ordering() {
    loom::model(|| {
        // setup the map
        let map: CMap2<f64> = CMapBuilder::default()
            .n_darts(5)
            .add_attribute::<Weight>()
            .build()
            .unwrap();
        map.two_link(DartId(1), DartId(2));
        map.two_link(DartId(3), DartId(4));
        map.one_link(DartId(1), DartId(3));
        map.one_link(DartId(4), DartId(5));
        map.insert_vertex(VertexId(2), Vertex2(0.0, 0.0));
        map.insert_attribute(VertexId(2), Weight(33));
        let arc = loom::sync::Arc::new(map);
        let (m1, m2) = (arc.clone(), arc.clone());

        // we're going to do to sew ops:
        // - 1-unsew 1 and 3 (t1)
        // - 2-unsew 3 and 4 (t2)
        // this will result in 3 different weights, defined on IDs 2, 3 and 5
        // depending on the order of execution, the final weights will take the following values:
        // 1-unsew before 2-unsew: (W2, W3, W5) = (17, 8, 8)
        // 2-unsew before 1-unsew: (W2, W3, W5) = (9, 8, 16)

        let t1 = loom::thread::spawn(move || {
            m1.atomically_one_unsew(DartId(1));
        });

        let t2 = loom::thread::spawn(move || {
            m2.atomically_two_unsew(DartId(3));
        });

        t1.join().unwrap();
        t2.join().unwrap();

        // all path should result in the same topological result here
        assert!(arc.get_attribute::<Weight>(VertexId(2)).is_some());
        assert!(arc.get_attribute::<Weight>(VertexId(3)).is_some());
        assert!(arc.get_attribute::<Weight>(VertexId(5)).is_some());
        let w2 = arc.get_attribute::<Weight>(VertexId(2)).unwrap();
        let w3 = arc.get_attribute::<Weight>(VertexId(3)).unwrap();
        let w5 = arc.get_attribute::<Weight>(VertexId(5)).unwrap();

        // check scenarios
        let path1 = w2.0 == 17 && w3.0 == 8 && w5.0 == 8;
        let path2 = w2.0 == 9 && w3.0 == 8 && w5.0 == 16;
        assert!(path1 || path2);
    });
}
