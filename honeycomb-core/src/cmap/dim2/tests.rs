...
fn two_sew_no_attributes() {
    let mut map: CMap2<f64> = CMap2::new(2);
    map.force_two_sew(1, 2); // should panic
}
...
fn one_sew_no_attributes() {
    let mut map: CMap2<f64> = CMap2::new(2);
    map.force_one_sew(1, 2); // should panic
}
...
fn io_write() {
    ...
    cmap.force_one_link(4, 1);
    // bottom right triangles
    cmap.force_one_link(5, 6);
    cmap.force_one_link(6, 7);
    cmap.force_one_link(7, 5);
    cmap.force_two_link(7, 8);
    cmap.force_one_link(8, 9);
    cmap.force_one_link(9, 10);
    cmap.force_one_link(10, 8);
    // top polygon
    cmap.force_one_link(11, 12);
    cmap.force_one_link(12, 13);
    cmap.force_one_link(13, 14);
    cmap.force_one_link(14, 15);
    cmap.force_one_link(15, 16);
    cmap.force_one_link(16, 11);
    // assemble
    cmap.force_two_link(2, 10);
    cmap.force_two_link(3, 11);
...
fn sew_ordering() {
    loom::model(|| {
        ...
        let t1 = loom::thread::spawn(move || {
            m1.force_one_sew(1, 3);
        });

        let t2 = loom::thread::spawn(move || {
            m2.force_two_sew(3, 4);
        });
...
fn unsew_ordering() {
    loom::model(|| {
        ...
        let t1 = loom::thread::spawn(move || {
            m1.force_one_unsew(1);
        });

        let t2 = loom::thread::spawn(move || {
            m2.force_two_unsew(3);
        });
