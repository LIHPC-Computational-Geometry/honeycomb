...
fn example_test() {
    ...
    map.unsew::<2>(2).unwrap(); // this makes dart 2 and 4 free
...
fn two_sew_bad_orientation() {
    ...
    map.link::<1>(3, 4).unwrap();
    map.write_vertex(1, (0.0, 0.0));
    map.write_vertex(2, (0.0, 1.0)); // 1->2 goes up
    map.write_vertex(3, (1.0, 0.0));
    map.write_vertex(4, (1.0, 1.0)); // 3->4 also goes up
    assert!(
        map.sew::<2>(1, 3)
