...
fn get_sew_map(n_square: usize) -> CMap2<FloatType> {
    let map = CMapBuilder::<2>::from_n_darts(n_square.pow(2) * 4)
        .build()
        .unwrap();
    map.write_vertex(4, (0.0, 0.0));
    map.write_vertex(6, (1.0, 0.0));
    map
}
